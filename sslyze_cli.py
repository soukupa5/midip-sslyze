...
from sslyze.plugins.common_new_plugin_info import ServerTestWithNonSSLCommandError
...
class CommandLineParser(object):
    ...
    REGULAR_CMD = ['sslv2', 'sslv3', 'tlsv1', 'tlsv1_1', 'tlsv1_2', 'reneg', 'resum', 'certinfo_basic', 'http_get',
                   'hide_rejected_ciphers', 'compression', 'heartbleed', 'openssl_ccs', 'fallback', 'beast', 'crime', 'poodle', 'lucky13_tls']
    ...
    def _add_default_options(self):
        ...
        self._parser.add_option(
            ...
            action="store_true",
            dest='verbose',
            help='Optional setting when you can see more information during scans')
...
NON_SSL_COMMANDS = {'shellshock', 'lucky13_dtls'}
def is_task_only_non_ssl_tests(args_command_list, available_commands):
    for command in available_commands:
        if getattr(args_command_list, command) and command not in NON_SSL_COMMANDS:
            return False
    return (getattr(args_command_list, 'shellshock') or getattr(args_command_list,'lucky13_dtls'))

def get_use_non_ssl_commands(args_command_list):
    use_cmd = ''
    for command in NON_SSL_COMMANDS:
        if getattr(args_command_list, command):
            use_cmd = use_cmd + ('--' + command + ' ')
    return use_cmd
def is_used_some_non_ssl_test_command(args_command_list):
    for command in NON_SSL_COMMANDS:
        if getattr(args_command_list,command):
            return True
    return False
def main():
    ...
    SERVER_ADDITIONAL_INFO_FORMAT = ' ADDITIONAL INFO: Can\'t use command {commands} with other commands which test SSL/TLS protocols on server.'
        ...
        for plugin_command in available_commands:
            if getattr(args_command_list, plugin_command):
                ...
                if args_command_list.verbose:
                    plugin_options_dict['verbose'] = getattr(args_command_list,'verbose')
    ...
    for tentative_server_info, exception in connectivity_tester.get_invalid_servers():
        #If user set option shellshock then invalid servers from connectivity tester are added.
        if is_task_only_non_ssl_tests(args_command_list, available_commands):
            tentative_server_info.port = 80
            ...
            for command in NON_SSL_COMMANDS:
                if getattr(args_command_list, command):
                    ...
                    for option in available_commands[command].get_interface().get_options():
                        if getattr(args_command_list,option.dest):
                            plugin_options_dict[option.dest]= getattr(args_command_list,option.dest)
                    plugins_process_pool.queue_plugin_task(tentative_server_info, command, plugin_options_dict)
        elif is_used_some_non_ssl_test_command(args_command_list):
            commands = get_use_non_ssl_commands(args_command_list)
            invalid_servers_list.append(('{}:{}'.format(tentative_server_info.hostname, tentative_server_info.port), ServerTestWithNonSSLCommandError(exception.error_msg,commands)))
        else:
    ...
    if should_print_text_results:
        for server_string, exception in invalid_servers_list:
            ...
            elif isinstance(exception, ServerTestWithNonSSLCommandError):
                print '{}{}'.format(
                    SERVER_INVALID_FORMAT.format(server_string=server_string, error_msg=exception.error_msg),
                    SERVER_ADDITIONAL_INFO_FORMAT.format(commands=exception.additional_msg))
            else:
    ...
    if args_command_list.json_file:
        ...
        for server_string, exception in invalid_servers_list:
            if isinstance(exception, ServerConnectivityError):
                ...
                json_output['invalid_targets'].append({server_string: exception.error_msg, 'wrong_commands' : exception.additional_msg})
            else:
                # Unexpected bug in SSLyze
                raise exception
    ...
    if args_command_list.xml_file:
        ...
        for server_string, exception in invalid_servers_list:
            if isinstance(exception, ServerConnectivityError):
                ...
                error_xml = Element('invalidTarget', error=exception.error_msg, wrong_commands=exception.additional_msg)
                error_xml.text = server_string
                invalid_targets_xml.append(error_xml)
            else:
                # Unexpected bug in SSLyze
